...
class PlayerCard extends React.Component {
  ...
  next5MatchDetails() {
    return this.props.next5Matches.map(match => {
      ...
      let opponentName = this.props.teams.find(t => {
        ...
        .then(function(scores) {
          ...
          let scoreList = that.next5MatchDetails().map(team => {
            let score = team.isHome
              ? scores.h[team.difficulty - 1]
              : scores.a[team.difficulty - 1];
            return (
              <div
                className="prediction-bar border d-flex flex-column justify-content-between align-items-center"
                key={team.opponentName}
              >
                <div className="p-1 prediction-bar__opponent-name">
                  {team.opponentName}
                </div>
                <div
                  className={
                    "w-100 text-center prediction-bar__prediction prediction-bar__prediction--d" +
                    team.difficulty
                  }
                >
                  {score}
          ...
          let backBody = (
            <div className="d-flex flex-column">
              <div className="d-flex prediction-bar__title">
                <img
                  className="prediction-bar__title-logo"
                  src={that.props.logoURL}
                />
                <div className="align-self-center ml-1 prediction-bar__title-text">
                  Score Predictions
                </div>
              </div>
  ...
  render() {
    ...
    let playerTeam = this.props.teams.find(t => {
    ...
    return (
      <ReactCardFlip isFlipped={this.state.isFlipped}>
        <div
          ...
          className="card mx-auto player-card__container shadow mt-2 mb-4"
          key="front"
        >
          <div className="d-flex flex-column">
            <div className="player-card__front-body px-3 py-2 d-flex flex-column justify-content-around">
              ...
              <div className="d-flex justify-content-between mt-2">
                <div className="d-flex flex-column">
                  <div className="player-card-__player-name">
                    {player.full_name}
                  </div>
                  <div className="player-card-__team-name">
                    {
                      this.props.teams.find(t => {
                        return t.id == player.team;
                      }).name
              ...
              <div className="d-flex justify-content-between mt-2 align-items-center">
                <div
                  className="player-card__bookmark-container d-flex shadow"
                  onClick={_e => this.props.toggleBookmarkCB(player)}
                >
                  {player.bookmarked ? (
                    <FontAwesomeIcon
                      icon={faHeartSolid}
                      className="red-text m-auto"
                    />
                  ) : (
                    <FontAwesomeIcon
                      icon={faHeart}
                ...
                <div
                  className="player-card__flip-btn px-2 py-1 shadow rounded"
                  onClick={this.flip}
                >
                  Predictions&nbsp;
                  <FontAwesomeIcon
